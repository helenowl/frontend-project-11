...
export default (watchedObj, i18n, path, value) => {
  ...
  if (path === 'feeds') {
    if (feedBox.children.length === 0) {
      feedBox.append(renderFeeds(watchedObj.feeds))
      return
    }
    const ul = document.querySelector('.feeds .list-group')
    const feedId = Array.from(ul.children).map(li => li.dataset.id)
    const onlyNewFeed = watchedObj.feeds.filter(({ id }) => !feedId.includes(id))
  ...
  if (path === 'posts') {
    if (postBox.children.length === 0) {
      postBox.append(renderPosts(watchedObj.posts))
      return
    }
    const ul = document.querySelector('.posts .list-group')
    const postsId = Array.from(ul.children).map(li => li.dataset.id)
    const onlyNewPost = watchedObj.posts.filter(({ id }) => !postsId.includes(id))
  ...
  if (path === 'uiState.modal') {
    const markedPosts = watchedObj.uiState.modal.filter(({ readIt }) => readIt)
      .map(({ id }) => id)
  ...
  if (path === 'uiState.selectedPost') {
    const idForTitleModal = watchedObj.uiState.selectedPost
    const { posts } = watchedObj
